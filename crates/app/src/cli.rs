...
pub(crate) struct CliRequestRow {
    pub(crate) id: String,
...
#[derive(Debug, Serialize)]
pub(crate) struct CliReplayResult {
...
pub(crate) fn request_rows_for_session(
    store: &Store,
    session_id: &str,
) -> anyhow::Result<Vec<CliRequestRow>> {
...
pub(crate) fn find_request_with_response(
    store: &Store,
    request_id: &str,
...
pub(crate) fn load_body(store: &Store, body_id: Option<&str>) -> anyhow::Result<Option<CliBody>> {
...
pub(crate) fn request_matches_filter(row: &CliRequestRow, expr: &str) -> bool {
    let expr = expr.trim();
    if expr.is_empty() {
        return true;
    }
